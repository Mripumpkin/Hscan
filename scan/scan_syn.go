...
func (s *SynScanner) scanHost(job hostJob) (Result, error) {
	...
	routes, err := netlink.RouteGet(job.ip)
	if err != nil {
		fmt.Println(err)
		return result, err
	}
	...
	if len(routes) == 0 {
		fmt.Println("No route found for IP:", job.ip)
		return result, fmt.Errorf("no route found for IP")
	}
	...
	gateway := route.Gw
	srcIP := route.Src
	networkInterface, err := net.InterfaceByIndex(route.LinkIndex)
	if err != nil {
	...
	if len(networkInterface.HardwareAddr) != 6 {
		fmt.Println("Warning: Hardware address is not 6 bytes long!")
	}
